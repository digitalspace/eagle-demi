...
Start both apps first, in two shells, from the repo root:

```
cd frontend && yarn start --host 127.0.0.1 --port 4200
cd frontend-react && yarn dev --host 127.0.0.1 --port 4300
```

Then, from `frontend-react/`:
