  ...
  for (const id of RECORD_TYPES) {
    it(`puts every ${id} filter id on the wire as and[<id>]=`, () => {
      ...
      for (const filterId of ids) {
        expect(query, `${id} is missing and[${filterId}]`).toContain(`&and[${filterId}]=v-${filterId}`);
      }
      expect(query.startsWith(`search?dataset=${config.dataset}&keywords=site`)).toBe(true);
      expect(query.endsWith('&fuzzy=false')).toBe(true);
    });
  }
});
