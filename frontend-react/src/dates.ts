 ...
 * offset, so BC would see "13 Oct" for a decision dated the 14th. Angular's date pipe reads it as
 * local midnight; this does the same. Null for anything that will not parse, so callers render
 * nothing, as the pipe does.
 */
...
/** Angular's `date:'d MMM y'`: "5 Sep 2024", the day unpadded. */
...
/** Angular's `date:'d MMM y, HH:mm'`: "5 Sep 2024, 14:03", the clock on 24 hours. */
...
/**
 * Angular's `currency:'CAD':'symbol':'1.4-4'` under the app's default en-US locale, where the CAD
 * symbol is "CA$". Four decimals because a single answer costs a fraction of a cent.
 */
