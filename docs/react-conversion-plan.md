...
- Camera: `flyTo` from a BC-wide view, `easeTo` otherwise, `fitBounds` on boundary click, all through the `flyOptions()` reduced-motion helper. Viewport project list from `map.getBounds()` on `moveend`, debounced 150 ms.
