...
- [x] ~~eagle-api serves `CONTENT_SEARCH` from `/api/config`~~ Done in code: eagle-api #853 (merged 2026-08-27). Tag v2.10.69 is burned (dev build failed, stale `ci-latest`). The dev build fails on the Trivy gate: Alpine `libcrypto3`/`libssl3` 3.5.7-r0 (HIGH ×2, fixed 3.5.8-r0) — bump the base image / `apk upgrade`, then cut v2.10.70 on test, then prod with the next release. Turning the tab on = `db.epic.updateOne({_schemaName:'Config'},{$set:{CONTENT_SEARCH:true}})` on that env's Mongo, nothing to deploy.
