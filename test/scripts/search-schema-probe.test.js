 ...
 * healthy index and read as drift; a 404 that failed would block the very deploy that adds the
 * endpoint; a 503 that passed would ship the outage this gate exists to prevent.
 */
...
test('search-schema-probe.sh', async (t) => {
  ...
  await t.test('passes on 404 — the running app predates the endpoint', async () => {
    ...
    assert.strictEqual(run.status, 0, run.stderr);
    // Silence would be the trap: the first rollout passes for a reason the log has to state.
    assert.match(run.stdout, /predates the endpoint/);
  });
