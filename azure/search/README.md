...
An index can carry every field the app selects and still hold nothing in them. On 2026-09-17 the
public project list showed no phase and no decision for all 359 production projects:
`currentPhaseName` and `eacDecision` were null on every row of the `projects` index, because
production Cosmos held those List references as bare id strings and the data source projects
`c.currentPhaseName.name`. The schema probe was green throughout — the fields were there, empty.
...
```json
{ "index": "projects", "field": "currentPhaseNameId", "filter": "currentPhaseNameId eq null", "max": 0 }
```
...
against production every six hours, which is what catches the rows going empty between deploys — the
2026-09-17 failure appeared days after the release that was serving it.

Adding a check means adding an entry. The filter has to name the field, and the field has to be
`filterable: true` in the index definition, or the query is a 400 the endpoint reports as
