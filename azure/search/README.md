...
`scripts/demi-devbox.sh apply --datasources demi-<container>-ds` does the same three steps in one
run. Pass `--datasources`: it PUTs the named data sources before the dry run, which a new indexer
otherwise fails — the dry run refuses while the data source its definition names is missing.
