...
@Component({
  ...
  template: `
    <div style="position: fixed; inset: 0; z-index: 950; background: rgba(0,0,0,0.35); display: grid; place-items: center; padding: var(--layout-padding-large);" (click)="closed.emit()">
      <div role="dialog" aria-label="How this screen is built" (click)="$event.stopPropagation()" style="width: 34rem; max-width: 100%; max-height: 80vh; overflow-y: auto; box-sizing: border-box; background: var(--surface-color-background-white); border-radius: var(--layout-border-radius-medium); box-shadow: 0 8px 30px rgba(0,0,0,0.3); padding: var(--layout-padding-large);">
...
export class HowBuiltComponent {
